...
<![endif]-->
<title><%=order.getOrderNo()  %>-<%=order.getCustName()  %></title>
</head>
<body class="pos-r">
<%
...
<div style="margin-left:2px;">
	<% if(!isPrintPage) {%>
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span class="c-gray en">&gt;</span> 查看订单 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<%} %>
	<div class="pd-20">
		<% 
		...
		<strong id="l_total_d" class="c-red">${order.prePay}</strong>元<%}%></span> 
		<span class="r">
			<a style="text-decoration:none" class="ml-5" <% if(isPrintPage) {%> onClick="window.print()"<%} %>  <% if(!isPrintPage) {%>href="javascript:window.open(location.href+'&act=print')" target="_blank"<%} %> title="打印当前订单">
		...
		<div class="row cl mt-20">
					<label class="form-label col-4 text-l"><span class="c-red">*</span>订单编号：${order.orderNo}</label>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>订单提交人：${order.creatorName}</label>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>提交时间：${order.createDate}</label>
		</div>
		
		<div class="row cl mt-20 ">
					<input type="hidden" id="nodeId" value="<%=curNodeId %>" />
					<input type="hidden" id="orderId" value="<%=order.getId() %>"/>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>客户名称：${order.custName}</label>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>当前处理人：${order.curOperatorName}</label>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>当前处理环节：<span class="label label-danger radius">${order.curNodeName}</span></label>
		</div>
		<div class="row cl  mt-20">
					<label class="form-label col-12 text-l"><span class="c-red">*</span>地址：${order.shipAddress}</label>
		</div>
		
		<div class="row cl mt-20 ">
					<label class="form-label col-8 text-l>
					<span class="c-red">*</span>付款账号：${order.payAccount}</label>
					<label class="form-label col-4 text-l"><span class="c-red">*</span>付款时间：${order.prePayTime}</label>
					
		</div>
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort"
				 id="mycart" style="padding-top:2px;">
				<thead>
					<tr class="text-c">
						<th width="10%">缩略图</th>
						<th width="10%">款号</th>
				...
				<tbody>
					<%
					    int s_cnt=0,m_cnt=0,l_cnt =0,xl_cnt=0,xxl_cnt=0;
