				...
				<c:if test="${prdlist!=null && fn:length(prdlist) > 0}">
					<c:forEach items="${prdlist}" var="t">   
						<tr class="text-c va-m">
							...
							<td><span class="price">
							<fmt:formatNumber type="number" value="${t.matWgt }" pattern="0.000" maxFractionDigits="3"/>  
							</span> </td>
							<td>
								<a style="text-decoration:none" class="ml-5" onClick="layershow(${t.Id},'${t.prdNo}');" href="javascript:;" title="编辑色号"><i class="Hui-iconfont">&#xe60c;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="shelf(${t.Id},'${t.prdNo}',${t.isVaild});" href="javascript:;" title="<c:if test="${t.isVaild == 0}">下架</c:if><c:if test="${t.isVaild == 1}">上架</c:if>"><i class="Hui-iconfont"><c:if test="${t.isVaild == 0}">&#xe6de;</c:if><c:if test="${t.isVaild == 1}">&#xe6dc;</c:if></i></a> 
								<a style="text-decoration:none" class="ml-5" onClick="edit(${t.Id},'${t.prdNo}');" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
							</td>
						</tr>
					 </c:forEach>
				 </c:if>
...
function goPage(pageNo){
	...
	if(pageNo){
		location.replace("products.do?prdName="+encodeURI(prdName)+"&pg="+pageNo);
	}else {
		location.replace("products.do?prdName="+encodeURI(prdName)+"&pg="+$('#pageNo').val());
	}
}
...
function shelf(prdId,prdNo,status){
	var tips = '';
	if(status== 0)
		tips = '下架';
	else tips = '上架';
	layer.confirm('确认要将产品'+prdNo+tips+'吗？',function(index){
		doshelf(tips,prdId,status)
	});
}
function edit(prdId,prdNo){
	var t=window.parent; 
	if(t)
	t.postMessage({id:'e_prd'+prdId,title:prdNo,url:'product.do?pid='+ prdId}, '*'); 
}
...
function layershow(prdId,prdNo){
	//layershow(title,url,w,h)
	var t=window.parent; 
	if(t)
		t.layershow(prdNo,"prdItems.do?pid="+prdId,800,500);
...
function doshelf(action,pid,status){
		$.ajax({ 
	        type:"get", 
	        url:"product/shelf.do?pid="+pid+"&status="+status, 
	        dataType:"json",      
	        contentType:"application/json", 
	        success:function(data){ 
